...
def server1_output_handler(server1_socket, client1_socket, client3_socket):
...
def server2_output_handler(server2_socket, client1_socket, client3_socket):
    global accessingResource
    global tasks
    global server_response
    global mutex
...
def input_handler(server1_socket, server2_socket, client1_socket, client3_socket):
    global timestamp
    global tasks
    global requests
    global release_states
    ...
    while True:
        ...
        if inp:
            ...
            if splitInput[0] == "lookup":
                ...
                timestamp += 1
                message = "Request (" + str(timestamp) + ", 2)\n"
                output = "Sending request (" + str(timestamp) + ", 2) to"
                release_states[(timestamp, 2)] = 0
                requests[(timestamp, 2)] = inp
                mutex.acquire()
                heappush(tasks, (timestamp, 2))
                mutex.release()
...
def heapHandler(client1_socket, client3_socket, server1_socket, server2_socket):
    global tasks
    global release_states
    global requests
    global accessingResource
...
def start_client():
    ...
    server1_socket.connect(('127.0.0.1', portnum+1)) # Connect to the localhost server at port 9999
    server2_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server2_socket.connect(('127.0.0.1', portnum+4)) # Connect to the localhost server at port 9999

    client1_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client1_socket.connect(('127.0.0.1', portnum+6)) # Connect to the localhost server at port 9999
    client3socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client3socket.bind(('127.0.0.1', portnum+8)) # specify host (itself) and port number 9999
    client3socket.listen(5)

    client3_socket, t = client3socket.accept()

    inputHandler = threading.Thread(target = input_handler, args = (server1_socket, server2_socket, client1_socket, client3_socket))
    ...
    heaper = threading.Thread(target = heapHandler, args = (client1_socket, client3_socket, server1_socket, server2_socket))
    heaper.daemon = True
    heaper.start()

    server1 = threading.Thread(target = server1_output_handler, args = (server1_socket, client1_socket, client3_socket))
    server1.daemon = True
    server1.start()

    server2 = threading.Thread(target = server2_output_handler, args = (server2_socket, client1_socket, client3_socket))
    server2.daemon = True
    server2.start()
